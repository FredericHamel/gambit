  ...
  (define (read-u8)
    (let ((ptr (vector-ref state 0))
      ...
      (if (< ptr (u8vector-length u8vect))
          ...
          (error "Error reading bytes 'read-u8"))))
  ...
  (define (deserialize-subprocedure-with-id! subproc-id)
    (let ((v (deserialize!)))
      (if (not (eq? v (##system-version)))
          (error "Invalid system version " v)
          (let* ((x
                  (read-u8))
                 (parent-name
                  (if (>= x (shared-tag))
